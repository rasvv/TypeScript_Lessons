...
export async function search(data: SearchFormData): Promise<FlatRentPlace[]> {
  ...
  const form = new FormData(document.forms.namedItem('search'))
...
export function collectSearchFormData() {
  ...
  const searchData: SearchFormData = {
    ...
    checkInDate: checkDate(new Date(searchString.get('checkin')), 2),
    checkOutDate: checkDate(new Date(searchString.get('checkout')), 4),
